...
from tasks.helpers.common import read_extracted_zone
from tasks.helpers.data_models import Metadata, TransformedField
...
def generate_days(metadata: Metadata, other: pd.DataFrame, days_path: str) -> pd.DataFrame:
    days = other.assign(
        # day_id=UUID,
...
def transform(metadata: Metadata) -> dict:
    ...
    for i, (json_path, zone_id) in enumerate(metadata.extract.historic_data):
        if i > 0:
            break
        logger.info('BEGIN transform for zone_id: %s', zone_id)
        tado_data = TadoDataModel.parse_file(json_path)
        logger.debug('JSON data parsed to TadoDataModel')

        days_path = generate_save_path(
            metadata.base_path, zone_id, metadata.date, ext=".csv", suffix="_days"
        )
        climate_path = generate_save_path(
            metadata.base_path, zone_id, metadata.date, ext=".csv", suffix="_climate"
        )
    ...
    metadata_new.transformed = TransformedField(
